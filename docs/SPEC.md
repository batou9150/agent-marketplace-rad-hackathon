...
21. **`SPEC-OPS-5`** : ✅ **Résolu**. Couverture de code mesurée à 94 % sur l'ensemble du projet (139 tests automatisés passant avec succès) via `pytest-cov`, atteignant 100 % sur les modules sensibles (`vibe_guard.audit.*`, `vibe_guard.rules.*`, `vibe_guard.report.masking`, `vibe_guard.report.models`, `vibe_guard.engine.snippet`, `vibe_guard.engine.models`, `vibe_guard.engine.scanner`), 93 % sur `vibe_guard.ingest.workspace`, et > 94 % sur `vibe_guard.report.builder` et `renderer`. Couvert par la suite d'intégration, `tests/unit/ingest/test_workspace_edge_cases.py` et `tests/unit/test_coverage_edge_cases.py`.
